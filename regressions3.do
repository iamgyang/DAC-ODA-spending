...
global regout1 "$input/total_regressions53.xls"
global regout2 "$input/pov_regressions53.xls"
...
foreach Y_outcome of local outcome_vars_local {
	...
	foreach regr of local regressions_toloop {
		...
		pause this is the regression loop
		...
		// top DAC countries:
			...
			foreach i of local countries_toloop {
				...
				// non-logistic regressions.
					...
					if (!strpos("`regr'", "Logistic") & !strpos("`i'", "DAC")) {
			...
			foreach i of local countries_toloop {
				...
				// drop missing variables na.omit
					...
					refugeepop_orig total wgi dist colony exports {
						drop if (`x' == .)
					}
				...
				// macros into the dataframe that we made previously.
						...
						qui:`regression_type' `Y_outcome' GDP Pop refugeepop_dest ///
						refugeepop_orig total wgi dist i.colony exports `regression_options'
							qui: fitstat
	...
	// Aesthetics
		...
		#delimit ;
			graph hbar (asis) pop_r2 ref_r2 politics_r2 full_r2, over(country, 
			sort(full_N) descending axis(lcolor(none))) bar(1, lcolor(none)) 
			bar(2, lcolor(none)) bar(3, lcolor(none)) bar(4, lcolor(none)) 
			blabel(bar, size(tiny) format(%7.2g)) yscale(range(0 1)) yscale(noline) 
			ylabel(0(.2)1, ticks nogrid) legend(position(12)) scheme(plotplainblind) 
			graphregion(fcolor(none) ifcolor(none)) plotregion(fcolor(none) 
			ifcolor(none)) title(`regr') subtitle(`outcome_variable_label')
		#delimit cr
	...
	} // ends the for each regression loop
} // ends the Y_outcome loop
