...
remotes::install_github("GIST-ORNL/wbstats")
list.of.packages <-
  c( "tidyverse", "ggpubr", "ggrepel", "extrafont", "wb_stats",
     "ggplot2", "data.table", "dplyr", "countrycode",
     "ggthemes", "rio", "RCurl", "foreign", "readxl", "XML",
     "dplyr", "forcats", "ggridges", "povcalnetR")
new.packages <-
...
China_spending <- setDT(crs)[year == 2019 &recipientname == "China (People's Republic of)" & donorname %in%c('Australia','Austria','United States','Germany','Japan','United Kingdom','France','Sweden','Netherlands','Canada','Norway','Switzerland','Italy','Korea','Denmark','Belgium','Spain','Ireland','Finland','New Zealand','Luxembourg','Poland','Portugal','Hungary','Czech Republic','Iceland','Greece','Slovak Republic','Slovenia') & sectorname != "Administrative Costs of Donors", .(sum(usd_grantequiv, na.rm = TRUE))] %>% unlist() %>% as.vector()

North_Korea_spending <- setDT(crs)[year == 2019 &recipientname == "Democratic People's Republic of Korea" & donorname %in%c('Australia','Austria','United States','Germany','Japan','United Kingdom','France','Sweden','Netherlands','Canada','Norway','Switzerland','Italy','Korea','Denmark','Belgium','Spain','Ireland','Finland','New Zealand','Luxembourg','Poland','Portugal','Hungary','Czech Republic','Iceland','Greece','Slovak Republic','Slovenia') & sectorname != "Administrative Costs of Donors", .(sum(usd_grantequiv, na.rm = TRUE))] %>% unlist() %>% as.vector()

Kosovo_spending <- setDT(crs)[year == 2019 &recipientname == "Kosovo" & donorname %in%c('Australia','Austria','United States','Germany','Japan','United Kingdom','France','Sweden','Netherlands','Canada','Norway','Switzerland','Italy','Korea','Denmark','Belgium','Spain','Ireland','Finland','New Zealand','Luxembourg','Poland','Portugal','Hungary','Czech Republic','Iceland','Greece','Slovak Republic','Slovenia') & sectorname != "Administrative Costs of Donors", .(sum(usd_grantequiv, na.rm = TRUE))] %>% unlist() %>% as.vector()
...
getpovhed <- function(rate_) {
  ...
  a <- rename(a, iso3c = countrycode,
            yr = year)
  a <- a[yr>=2010,c("iso3c", "yr", paste0("pov",rate_)),with=FALSE]
  # if both income and consumption measures for something exist, 
  # then take the mean of the two:
  a[,(paste0("pov",rate_)):= mean(unlist(.SD), na.rm=TRUE),by = iso3c,.SDcols = paste0("pov",rate_)]
  a <- distinct(a)
  a
}
...
exp_grid <- CJ(unique(dac$iso3c_d), list_all_isos) %>% as.data.table()
...
exp_grid <- merge(exp_grid[,.(iso3c_d = V1, iso3c = list_all_isos)], dac_collapsed, 
      all.x=TRUE,by = c("iso3c_d", "iso3c"))
...
waitifnot(length(pov1$iso3c)==length(unique(pov1$iso3c)))
waitifnot(length(pov2$iso3c)==length(unique(pov2$iso3c)))
waitifnot(length(dem$iso3c)==length(unique(dem$iso3c)))
waitifnot(length(wgi$iso3c)==length(unique(wgi$iso3c)))
before <- nrow(exp_grid)
exp_grid <- Reduce(function(d1, d2) merge(d1, d2, by = "iso3c",
                                     all.x = TRUE,
...
before <- nrow(exp_grid)
exp_grid <- Reduce(function(d1, d2) merge(d1, d2, by = c("iso3c", "iso3c_d"),
                                     all.x = TRUE,
                                     allow.cartesian = FALSE),
              df_list_2)
waitifnot(nrow(exp_grid) == before)
